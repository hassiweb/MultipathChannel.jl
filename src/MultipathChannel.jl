...
function initChan(maxdoppler, samplingrate, channelmodel; rmsdelay=300, maxorder=8)
    ...
    return ChanPars(numpaths, pathdelays, pathdopplers, pathgains, pathoffsets, fdfcoefs, nonzeroindices, samplingrate)
end
...
function applyChan(in, chan::ChanPars; initialtime=0.0)
    ...
    for n = 1:chan.numpaths
        ...
        @. rayleighwave += √(10^(chan.pathgains[n]/10)) * (cos(2π*chan.pathdopplers[n]*t-chan.pathoffset[n]) + im*sin(2π*chan.pathdopplers[n]*t-chan.pathoffset[n])) * delayedwave
    end
    return rayleighwave
end